...
from torchwright.debug.probe import (
    attention_capture, probe_attention, probe_layer_diff, probe_residual,
)
...
def _find_raw_sel_bsp_rank(effective_node):
    ...
    assert isinstance(node, Linear), f"expected select's output Linear, got {type(node).__name__}"
    ...
    assert isinstance(first_linear, Linear), f"expected Linear, got {type(first_linear).__name__}"
    # First Linear -> its input is Concatenate([cond, true, false])
    concat = first_linear.inputs[0]
    assert isinstance(concat, Concatenate), f"expected Concatenate, got {type(concat).__name__}"
    assert len(concat.inputs) == 3, f"expected 3 inputs to select's Concatenate, got {len(concat.inputs)}"
...
def main():
    ...
    print(f"  sel_eff:   {type(sel_effective).__name__} -> {type(sel_effective_inner).__name__}")
    print(f"  sort_done: {type(sort_done).__name__} -> {type(sort_done_inner).__name__}")
    ...
    report_done = probe_layer_diff(
        module, sort0_in, sort_done_inner,
        reference=torch.zeros(1, 1),
        positions=[0],
        past_len=step,
        past_kvs=past_kvs,
    ...
    report_eff = probe_layer_diff(
        module, sort0_in, sel_effective_inner,
        reference=torch.zeros(1, 1),
        positions=[0],
        past_len=step,
        past_kvs=past_kvs,
    ...
    print(f"  found Attn: value_width={len(attn_node.inputs[2]) if attn_node.inputs[2] else 'None'}")
    ...
    attn_probe = probe_attention(
        module, sort0_in, attn_node,
        query_pos=0,
        past_len=step,
        past_kvs=past_kvs,
        position_labels=labels,
    )
    print(f"  attention at layer {attn_probe.layer_index}, n_heads={attn_probe.weights.shape[0]}")
    ...
    for head_i in range(attn_probe.weights.shape[0]):
        ...
        wall_mass = 0.0
        for idx, weight, label in [(i, w[i].item(), labels[i] if i < len(labels) else f"pos{i}")
                                    for i in range(w.shape[0])]:
            if label.startswith("WALL["):
                wall_mass += weight
        sorted_mass = sum(w[i].item() for i in range(w.shape[0])
        ...
        for pos_k, weight_k, label_k in top:
            ...
            print(f"      pos={pos_k:3d}  logit={logit_k:+12.4f}  weight={weight_k:.6f}  {label_k}")
    ...
    for w_i in range(n_walls):
        ...
        print(f"    pos={pos:3d}  logit={logit:+12.4f}  weight={weight:.6f}  {labels[pos]}")
    ...
    print(f"  indicators_above node: {type(indicators_node).__name__}  width={len(indicators_node)}")
    ...
    if ref_vals:
        ...
        for pos in [wall_start + i for i in range(n_walls)] + [n_prefill - 1]:  # walls + EOS
            label = labels[pos] if pos < len(labels) else f"pos{pos}"
            s = score_ref[pos, 0].item()
            ind = ind_ref[pos].tolist()
            print(f"    pos={pos:3d}  {label:25s}  score={s:+8.2f}  ind_above={[f'{v:.2f}' for v in ind]}")
    ...
    if score_probe.value is not None and ind_probe.value is not None:
        for pos in [wall_start + i for i in range(n_walls)] + [n_prefill - 1]:
            ...
            print(f"    pos={pos:3d}  {label:25s}  score={s:+8.2f}  ind_above={[f'{v:+5.2f}' for v in ind]}")

    # Summary
    raw_final = report_raw.records[-1].value[0, 0].item() if report_raw.records else None
    done_final = report_done.records[-1].value[0, 0].item() if report_done.records else None
    eff_final = report_eff.records[-1].value[0, 0].item() if report_eff.records else None
    ...
    if raw_final is not None:
        ...
        print(f"  prev - raw = {prev - raw_final:+.4f} → sort_done expected = {expected_sort_done:+.0f}")
