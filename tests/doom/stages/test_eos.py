...
def eos_module():
    ...
    output = Concatenate([
        out.resolved_x, out.resolved_y, out.px, out.py, out.angle,
    ])
...
def _wall_row(hit_full: bool, hit_x: bool, hit_y: bool, px, py, vdx, vdy, angle) -> dict:
...
def _eos_row(px, py, vdx, vdy, angle) -> dict:
    return {
        "hit_full": -1.0, "hit_x": -1.0, "hit_y": -1.0,  # gated anyway
        "is_wall": 0.0,
        "is_eos": 1.0,
        "player_x": px, "player_y": py,
        "vel_dx": vdx, "vel_dy": vdy,
...
@pytest.mark.parametrize("scenario", [
    ...
    dict(
        ...
        px=1.0, py=2.0, vdx=0.2, vdy=0.1,
        expected_x=1.2, expected_y=2.1,
    ),
...
def test_collision_resolution_matches_logic(eos_module, scenario):
    ...
    wall_rows = [
        _wall_row(
            hf, hx, hy,
            px=scenario["px"], py=scenario["py"],
            vdx=scenario["vdx"], vdy=scenario["vdy"],
            angle=0.0,
        )
...
def test_state_broadcast_reaches_non_eos_positions(eos_module):
    ...
    walls = [_wall_row(False, False, False,
                       px=3.0, py=4.0, vdx=0.1, vdy=0.0, angle=42.0)]
    ...
    receiver = _wall_row(False, False, False,
                         px=0.0, py=0.0, vdx=0.0, vdy=0.0, angle=0.0)
