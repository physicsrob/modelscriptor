...
def sorted_module():
    ...
    sort_score = assert_integer(create_input("sort_score", 1, value_range=(0.0, 100.0)))
    sort_value = create_input(
        "sort_value", payload_w, value_range=(-_MAX_COORD, _MAX_COORD)
    )
...
def test_sort_done_progress_signal(
    ...
    assert (
        abs(got_sel_rank - expected_sel) < 0.3
    ), f"{label}: sel_bsp_rank={got_sel_rank:+.3f}, expected {expected_sel}"
    assert (
        got_sort_done * expected_sort_done > 0.5
