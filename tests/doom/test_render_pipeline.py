...
def _build_sort_precomp(pos):
    ...
    ey_cos = piecewise_linear_2d(
        ey, player_cos, _DIFF_BP, _TRIG_BP, lambda a, b: a * b, name="ey_cos_p"
    )
    ex_sin = piecewise_linear_2d(
        ex, player_sin, _DIFF_BP, _TRIG_BP, lambda a, b: a * b, name="ex_sin_p"
    )
    sort_den = subtract(ey_cos, ex_sin)

    ey_sin = piecewise_linear_2d(
        ey, player_sin, _DIFF_BP, _TRIG_BP, lambda a, b: a * b, name="ey_sin_p"
    )
    ex_cos = piecewise_linear_2d(
        ex, player_cos, _DIFF_BP, _TRIG_BP, lambda a, b: a * b, name="ex_cos_p"
    )
    C_val = add(ey_sin, ex_cos)

    fx_sin = piecewise_linear_2d(
        fx, player_sin, _DIFF_BP, _TRIG_BP, lambda a, b: a * b, name="fx_sin_p"
    )
    gy_cos = piecewise_linear_2d(
        gy, player_cos, _DIFF_BP, _TRIG_BP, lambda a, b: a * b, name="gy_cos_p"
    )
    D_val = add(fx_sin, gy_cos)

    fx_cos = piecewise_linear_2d(
        fx, player_cos, _DIFF_BP, _TRIG_BP, lambda a, b: a * b, name="fx_cos_p"
    )
    gy_sin = piecewise_linear_2d(
        gy, player_sin, _DIFF_BP, _TRIG_BP, lambda a, b: a * b, name="gy_sin_p"
    )
    E_val = subtract(fx_cos, gy_sin)

    ey_fx = piecewise_linear_2d(
        ey, fx, _DIFF_BP, _DIFF_BP, lambda a, b: a * b, name="ey_fx"
    )
    ex_gy = piecewise_linear_2d(
        ex, gy, _DIFF_BP, _DIFF_BP, lambda a, b: a * b, name="ex_gy"
    )
    num_t = add(ey_fx, ex_gy)
    abs_num_t = node_abs(num_t)
    inv_abs_num_t = reciprocal(
        abs_num_t, min_value=0.3, max_value=2.0 * MAX_COORD * MAX_COORD, step=1.0
    )
...
class TestRenderIntermediate:
    ...
    def module(self):
        ...
        output = Concatenate(
            [
                p["sort_den"],
                p["C"],
                p["num_t"],  # 3: precomputed values
                doc,
                abs_doc,  # 2: per-column intermediates
        ...
        return compile_headless(
            output, pos, d=2048, d_head=32, max_layers=100, verbose=False
        )
    ...
    def test_near_dist2(self, module):
        ...
        assert (
            builtins.abs(r["wall_height"] - 12.0) < 2.0
        ), f"wall_height should be ~12, got {r['wall_height']:.3f}"
        assert (
            builtins.abs(r["wall_top"] - 6.0) < 2.0
        ), f"wall_top should be ~6, got {r['wall_top']:.3f}"
        assert (
            builtins.abs(r["wall_bottom"] - 18.0) < 2.0
    ...
    def test_oblique_wall(self, module):
        ...
        assert r["u_raw"] >= 0.0, f"u_raw should be non-negative, got {r['u_raw']:.3f}"
...
class TestRenderPixels:
    ...
    def module_and_textures(self):
        ...
        module = compile_headless(
            output, pos, d=2048, d_head=32, max_layers=200, verbose=False
        )
        return module, texs
    ...
    def test_near_wall_pixel_output(self, module_and_textures):
        ...
        for r in range(H):
            ...
            print(
                f"  row {r:2d}: [{rgb[0]:.3f}, {rgb[1]:.3f}, {rgb[2]:.3f}]  "
        ...
        assert (
            builtins.abs(wall_height - 12.0) < 2.0
    ...
    def test_center_wall_pixel_output(self, module_and_textures):
        ...
        assert (
            builtins.abs(wall_height - 4.8) < 1.5
